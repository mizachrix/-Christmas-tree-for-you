function[sol] = solve_Y_ADMM_3_CC(si,input, Input_param)
...
%     
    ...
    Obj = trace(Y'*si.Ag*Y*si.exp_xi_xit) + si.Cg'*Y*[1;zeros(size(si.mu,1),1)] + trace(input.lambda' * (Xc)) + input.rho/2 * trace( ((Xc) - (input.K * input.X))' * ((Xc) - (input.K * input.X)) ); 
