function[sol] = solve_X2_CC(si,input, Input_param)
    ...
    % This script solve the initialization electricity problem
...
%     CS = [CS, sum(Z, 1) == sum(si.PG * Y, 1)];
    ...
    for i = 1:Nunits
        CS = [CS, norm(X(i,2:Nwind+1) * si.cov^(1/2),2) <= CC_margin * (si.Pmax(i) - X(i,1)')];
        CS = [CS, norm(-X(i,2:Nwind+1) * si.cov^(1/2),2) <= CC_margin * (si.Pmin(i) - (-X(i,1))')];
    end
    for i = 1:Nlines
        CS = [CS, norm(si.Qg(i,:) * X(:,2:Nwind+1) + si.Qw(i,:)*si.DiagWmax * si.cov^(1/2),2) <= CC_margin * (si.F(i) - (si.Qg(i,:) * X(:,1) + si.Qw(i,:)*si.DiagWmax*si.mu - si.Qd(i,:)*si.D)')];
        CS = [CS, norm( -(si.Qg(i,:) * X(:,2:Nwind+1) + si.Qw(i,:)*si.DiagWmax) * si.cov^(1/2),2) <= CC_margin * (si.F(i) - (-(si.Qg(i,:) * X(:,1) + si.Qw(i,:)*si.DiagWmax*si.mu - si.Qd(i,:)*si.D))')];
    end
...
%     CS = [CS, - repmat(si.FG, 1, Nscen) <=  Q * si.xi2 <= repmat(si.FG, 1, Nscen)];
    ...
    Obj = trace(X'*si.A*X*si.exp_xi_xit) + si.C'*X*[1;si.mu] - trace(input.lambda' * (diag(si.phi) * X) * si.exp_xi_xit);
